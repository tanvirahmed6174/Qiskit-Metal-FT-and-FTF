...
class TransmonCross_mod(BaseQubit):  # pylint: disable=invalid-name
...
###################################TRANSMON#############################################################
    ...
    def make_pocket(self):
        ...
        cross_line = draw.shapely.ops.unary_union([
            draw.LineString([(0, cross_length), (0, -cross_length)]),draw.LineString([(1/2*cross_length, cross_length), (1/2*cross_length, -cross_length)]),draw.LineString([(cross_length, cross_length), (cross_length, -cross_length)]),
            # draw.LineString([(3*cross_length, 0), (-cross_length, 0)]), 
            draw.LineString([(1.5*cross_length, 0), (0, 0)]), 
            # draw.LineString([(cross_width/2, 2/3*cross_length-cross_width ), (-cross_length-cross_width, 2/3*cross_length-cross_width)]),
            draw.LineString([(cross_width/2, 1/4*cross_length ), (-cross_length-cross_width, 1/4*cross_length)]),
            draw.LineString([(cross_width/2, 3/4*cross_length ), (-cross_length-cross_width, 3/4*cross_length)]),
            # draw.LineString([(cross_width/2, -cross_length+cross_width), (-cross_length-cross_width, -cross_length+cross_width)])
            draw.LineString([(cross_width/2, -1/4*cross_length), (-cross_length-cross_width, -1/4*cross_length)]),
            draw.LineString([(cross_width/2, -3/4*cross_length), (-cross_length-cross_width, -3/4*cross_length)])
        ])
        ...
        rect_jj = draw.LineString([(1.5*cross_length,0),
                                   ( 1.5*cross_length + cross_gap,0)])
    ...
    def make_connection_pad(self, name: str):
        ...
        if pc.connector_type == 0:  # Claw connector
            ...
            claw_subtract_1 = draw.box(0, -t_claw_height / 2 + c_w, c_l,
                                     t_claw_height / 2 - c_w) # middle cut
            claw_subtract_2 = draw.box(c_w, -t_claw_height / 2 - cross_length + c_w, c_l,
