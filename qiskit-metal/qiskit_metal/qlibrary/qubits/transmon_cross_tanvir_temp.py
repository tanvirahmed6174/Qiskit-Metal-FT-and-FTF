...
class TransmonCross_tanvir_temp(BaseQubit):  # pylint: disable=invalid-name
    ...
    default_options = Dict(
        ...
        _default_connection_pads=Dict(
            ...
            lead_gap='10um',
            connector_location='0'),# 0 => 'west' arm, 90 => 'north' arm, 180 => 'east' arm
        _default_extra_rect=Dict(make_extra_rect=False, rect_options=Dict(rect_width='30um', rect_length='10um'))
        )
    ...
    def make(self):
        ...
        if self.options.make_extra_rect == True:
            self.make_pocket_extra()
        else: 
            self.make_pocket()
...
###################################TRANSMON#############################################################
    ...
    def make_pocket_extra(self):
        """Makes a basic Crossmon, 4 arm cross with an additional rectangular capacitance at the end ."""

        # self.p allows us to directly access parsed values (string -> numbers) form the user option
        p = self.p
        pr = self.p.rect_options
        rect_width = pr.rect_width
        rect_length = pr.rect_length 
        ...
        cross_line_etch = draw.shapely.ops.unary_union([
            draw.LineString([(0, cross_length), (0, -cross_length)]),
            draw.LineString([(cross_length, 0), (-cross_length*1.5, 0)])
            ])    

        
        extra_etch = draw.LineString([(-cross_length, -cross_length), (cross_length, -cross_length)])
        extra = extra_etch.buffer(cross_gap, cap_style=2)
    
        cross = cross_line.buffer(cross_width / 2, cap_style=2)
        
        cross_e =  cross_line_etch.buffer(cross_width / 2, cap_style=2)
        cross_etch = cross_e.buffer(cross_gap, cap_style=3, join_style=2)
        ...
        # The junction/SQUID
        rect_line = draw.rectangle(rect_width, rect_length)
        rect_line = draw.translate(rect_line, 0, -cross_length+rect_length/2)
        rect_jj = draw.LineString([(0, -cross_length),
                                   (0, -cross_length - cross_gap)])

        #rotate and translate
        polys = [cross, cross_etch,  rect_jj, rect_line, extra]
        polys = draw.rotate(polys, p.orientation, origin=(0, 0))
        polys = draw.translate(polys, p.pos_x, p.pos_y)

        [cross, cross_etch, rect_jj, rect_line, extra] = polys
        cross = draw.union(cross, rect_line)
        # generate qgeometry
    ...
    def make_pocket(self):
        ...
        cross_line = draw.shapely.ops.unary_union([
            draw.LineString([(0, cross_length), (0, -cross_length)]),
            draw.LineString([(cross_length, 0), (-cross_length, 0)]),
            draw.LineString([(cross_width/2, cross_length ), (-cross_length, cross_length)]),
            draw.LineString([(cross_width/2, -cross_length), (-cross_length, -cross_length)])
        ])
...
############################CONNECTORS##################################################################################################
    ...
    def make_connection_pad(self, name: str):
        ...
        lead_width = pc.lead_width 
        lead_length = pc.lead_length
        lead_gap = pc.lead_gap

        claw_cpw = draw.box(0, -1*lead_width/2, -1*lead_length, lead_width/2)

        if pc.connector_type == 0:  # Claw connector
            t_claw_height = 2*c_g + 2 * c_w + 2*g_s + \
                2*cross_gap + cross_width  # temp value

            claw_base = draw.box(-c_w, (t_claw_height) / 2-cross_length-c_w , c_l ,
                                 c_w-t_claw_height / 2+ cross_length)
            claw_subtract_1 = draw.box(0, -t_claw_height / 2 + c_w, c_l,
                                     t_claw_height / 2 - c_w)
            claw_subtract_2 = draw.box(0, -t_claw_height / 2 - cross_length + c_w, c_l,
                                     t_claw_height / 2 - c_w - cross_length)
            claw_subtract_3 = draw.box(0, -t_claw_height / 2 + cross_length + c_w, c_l,
                                     t_claw_height / 2 - c_w + cross_length)
            claw_subtract_4 = draw.box(-c_w, t_claw_height / 2 , c_l-c_w,
                                     -t_claw_height / 2 + cross_length)
            claw_subtract_5 = draw.box(-c_w, -t_claw_height / 2 , c_l-c_w,
                                     t_claw_height / 2 - cross_length)
            claw_base = claw_base.difference(claw_subtract_1)
            claw_base = claw_base.difference(claw_subtract_2)
            claw_base = claw_base.difference(claw_subtract_3)
            claw_base = claw_base.difference(claw_subtract_4)
            claw_base = claw_base.difference(claw_subtract_5)

            connector_arm = draw.shapely.ops.unary_union([claw_base, claw_cpw])

            connector_etcher= draw.buffer(claw_base, c_g)
            #Ebru: I am changing this so that we can have separate dielectic gaps for the main and the cpw part. Might need to be reverted later
            etcher_cpw = draw.box(0, -1.0*(lead_gap+lead_width/2), -1.0*lead_length, (lead_gap+lead_width/2))
            connector_etcher = draw.union(connector_etcher, etcher_cpw)
        ...
        port_line = draw.LineString([(-1*lead_length, -1*lead_width/2), (-1*lead_length, 1*lead_width/2)])
