...
class TransmonCross_Tanvir_v3(BaseQubit):  # pylint: disable=invalid-name
...
###################################TRANSMON#############################################################
    ...
    def make_pocket_extra(self):
        ...
        # Create the circle geometry and combine it with the cross
        circle = Point(0, 0).buffer(circle_radius)  # Create the circle at the center
        cross = cross_line.buffer(cross_width / 2, cap_style=2)  # Buffer to create cross width
        cross = cross.union(circle)  # Add the circle to the cross geometry  
        # Create the circle etch
        circle_etch = circle.buffer(cross_gap)  # Buffer the circle to create the etch
        ...
        # Combine the circle etch with the cross etch
        cross_etch = cross_etch.union(circle_etch)  # Add the circle etch to the cross etch
    ...
    def make_pocket(self):
        ...
        cross_length = p.cross_length
        cross_gap = p.cross_gap
        circle_radius = p.circle_radius  # Circle radius (new parameter)
        ...
        cross = cross_line.buffer(cross_width / 2, cap_style=2)
         ...
         # Create the circle geometry and combine it with the cross
        circle = Point(0, 0).buffer(circle_radius)  # Circle at the center
        cross = cross.union(circle)  # Combine the cross and the circle
...
############################CONNECTORS##################################################################################################
    ...
    def make_connection_pad(self, name: str):
        ...
        port_line = draw.LineString([(-1*lead_length, -1*lead_width/2), (-1*lead_length, lead_width/2)])
