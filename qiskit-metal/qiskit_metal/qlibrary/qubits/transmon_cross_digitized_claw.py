...
class TransmonCross(BaseQubit):  # pylint: disable=invalid-name
    ...
    def make(self):
        ...
        component."""
        self.make_pocket('connect')
        # self.make_connection_pads()

###################################TRANSMON#############################################################

    def make_pocket(self, name:str):
        ...
        cross_line = draw.shapely.ops.unary_union([
                    #    draw.LineString([(0, 1*cross_length), (0, -0.5*cross_length)]),
                    
                    draw.LineString([(0, 1*cross_length), (0, -0.5*cross_length)]),

            draw.LineString([(2*cross_length, -.5*cross_length-0*cross_width), (-2*cross_length, -.5*cross_length-0*cross_width)]),
            # draw.LineString([(-0.9*cross_length, -0*cross_length), (0.9*cross_length, -0*cross_length)]),

            draw.LineString([(cross_length, 0.25*cross_length-0*cross_width), (-cross_length, 0.25*cross_length-0*cross_width)]),
            # draw.LineString([(-cross_length, 3/5*cross_length), (cross_length, 3/5*cross_length)]),
            draw.LineString([(-cross_length, cross_length), (cross_length, cross_length)]),
            # draw.LineString([(-3*cross_length, cross_length-cross_width/1.1), (3*cross_length, cross_length-cross_width/1.1)]),
            draw.LineString([(-2*cross_length, 0*cross_length), (-2*cross_length, -1*cross_length)]),
            draw.LineString([(2*cross_length, 0*cross_length), (2*cross_length, -1*cross_length)]),
            
            # draw.LineString([(-2*cross_length + cross_width/2, 0.5*cross_length), (-2.5*cross_length, 0.5*cross_length)]),
            # draw.LineString([(2*cross_length - cross_width/2, 0.5*cross_length), (2.5*cross_length, 0.5*cross_length)]),
            
            # draw.LineString([(-2.5*cross_length + cross_width, 1.5*cross_length), (-2.5*cross_length + cross_width, 0.5*cross_length)]),
            # draw.LineString([(2.5*cross_length + cross_width, 1.5*cross_length), (2.5*cross_length + cross_width, 0.5*cross_length)]),

            # draw.LineString([(-cross_length, -1.5*cross_length), (cross_length, -1.5*cross_length)])
        ...
        rect_jj = draw.LineString([(0, -.5*cross_length - cross_width/2 ),
                                   (0, -.5*cross_length -cross_width/2- cross_gap )])
       
       
        #added the port_line here_extra - Tanvir
        
        port_line = draw.LineString([(-cross_width/2,cross_length+cross_width/2 ),
                                   ( cross_width/2,cross_length+cross_width/2)])

        #rotate and translate
        polys = [cross, cross_etch, rect_jj,port_line]
        polys = draw.rotate(polys, p.orientation, origin=(0, 0))
        polys = draw.translate(polys, p.pos_x, p.pos_y)

        [cross, cross_etch, rect_jj,port_line] = polys
        ...
        self.add_pin(name, port_line.coords, cross_length)
