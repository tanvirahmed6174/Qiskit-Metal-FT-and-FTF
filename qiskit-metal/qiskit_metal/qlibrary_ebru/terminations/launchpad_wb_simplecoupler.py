...
class LaunchpadWirebondSimple(QComponent):
    ...
    def make(self):
        ...
        launch_pad = draw.Polygon([
            (-coupling_distance, trace_width_half), (-taper_height-coupling_distance, pad_width_half),
            (-(pad_height + taper_height+coupling_distance), pad_width_half),
            (-(pad_height + taper_height+coupling_distance), -pad_width_half),
            (-taper_height-coupling_distance, -pad_width_half), (-coupling_distance, -trace_width_half),
            (lead_length-coupling_distance, -trace_width_half), (lead_length-coupling_distance, trace_width_half),
            (-coupling_distance, trace_width_half)
        ])
#        ground_pad = draw.Polygon([(lead_length, trace_width/2  + trace_gap), (lead_length + trace_width/4, trace_width/2 + trace_gap ),  
#                                   (lead_length +trace_width/4, -trace_width/2 - trace_gap ),
#                                   (lead_length  , -trace_width/2 - trace_gap),(lead_length, trace_width/2 + trace_gap) ])
        # Geometry pocket (gap)
        # Same way applied for pocket
        #Ebru: Shifting the origin by coupling distance so that it does not get confused about patching the next transmission line element
        pocket = draw.Polygon([(-coupling_distance, trace_width_half + trace_gap),
                               (-taper_height-coupling_distance, pad_width_half + pad_gap_y),
                               (-(pad_height + taper_height + pad_gap_x+coupling_distance),
                                pad_width_half + pad_gap_y),
                               (-(pad_height + taper_height + pad_gap_x+coupling_distance),
                                -(pad_width_half + pad_gap_y)),
                               (-taper_height-coupling_distance, -(pad_width_half + pad_gap_y)),
                               (-coupling_distance, -(trace_width_half + trace_gap)),
                               (lead_length+trace_width/4, -(trace_width_half + trace_gap)),
                               (lead_length+trace_width/4, trace_width_half + trace_gap),
                               (-coupling_distance, trace_width_half + trace_gap)])
...
#
        ...
        main_pin_line = draw.LineString([(lead_length+trace_width/4, trace_width_half+trace_gap),
                                         (lead_length+trace_width/4, -trace_width_half-trace_gap)])
...
#        self.add_qgeometry('poly', dict(ground_pad=ground_pad), layer=p.layer)
